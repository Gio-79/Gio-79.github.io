...
`;
var structure1 = `1. Einleitung
Hinführung: Aktualität/ gesellschaftliche Relevanz des Themas
Textvorstellung:
- Titel
- Autor
- Textart (Kommentar, Essay, Leserbrief, Rede, Glosse)
- Thema
- Erscheinungsjahr/ Ort.
Kernaussage: Zentrale These/Problemvorstellung des Autors präzise formulieren.
Vorgehensweise: Leitfrage/methodisches Vorgehen ankündigen.

2. Hauptteil (A)
Inhaltswiedergabe (knapp):
- Wesentliche Aussagen zusammenfassen.
- Gedankengang/Argumentationsverlauf
Analyse der Argumentationsstruktur:
- Argumentationsaufbau (linear, dialektisch, steigernd)
- Argumenttypen identifizieren (Fakten-, Autoritäts-, Erfahrungs-, normative Argumente)
- Belege, Beispiele, rhetorische Mittel analysieren
Sprachlich-stilistische Analyse:
- Rhetorische Mittel (Metaphern, Wiederholungen, rhetorische Fragen, Ironie)
- Wirkung auf Leser und persuasive Strategien
- Zielgruppe und Adressatenbezug

3. Hauptteil (B)
Analyse der Argumentationslogik:
- Stichhaltigkeit, Schlüssigkeit, Belegqualität prüfen.
- Logische Fehler oder Lücken aufzeigen.
Kritische Auseinandersetzung mit den Thesen:
- Zustimmende Punkte begründet darlegen (mit eigenen Beispielen).
- Kritische Punkte/Kontra-Argumente entwickeln.
- Gegenpositionen berücksichtigen und abwägen.
- Mögliche Einseitigkeiten oder Überzeichnungen benennen.
Erweiternde Perspektiven:
- Aktualitätsbezug: Ist die Position heute noch haltbar?
- Gesellschaftliche/ethische Implikationen
- Eigene Standpunkte fundiert vertreten.

4. Schluss
Zusammenfassung: der Hauptergebnisse aus der Analyse und Erörterung.
Abwägendes Gesamturteil: Überzeugungskraft der Textposition bewerten.
Ausblick/Perspektive: Gesamtdeutung des Themas, Handlungsoptionen.
Evtl. persönliche Positionierung: (knapp und begründet)`;

var structure2 = `1. Einleitung
Hinführung: Allgemeine Gedanken zum zentralen Thema/Motiv des Textes.
Textangaben (T-A-T-T-E):
– Titel
– Autor
– Textart (z. B. Kurzgeschichte)
– Thema (knappe Inhaltsangabe im Präsens)
– Erscheinungsjahr/Epoche.
Deutungshypothese: Klare, begründete These zur Gesamtaussage als roter Faden.

2. Hauptteil
Inhalt und Figuren:
– Figurenanalyse (Charakterisierung, Motive, Entwicklung).
– Figurenkonstellation und Beziehungen (Konflikte, Abhängigkeiten).
– Zentrale Konflikte (innerlich oder zwischenmenschlich).
Erzähltechnik und Aufbau:
– Erzählperspektive und -verhalten (auktorial, personal, neutral).
– Darbietungsformen (Erzählerbericht, Redeformen, innerer Monolog).
– Struktur (Beginn, Wendepunkte, offenes Ende, Zeitgestaltung).
Sprache und stilistische Mittel:
– Wortwahl, Wortfelder und Satzbau (Parataxe/Hypotaxe).
– Rhetorische Mittel, Symbole und Leitmotive deuten.
Raum- und Zeitgestaltung:
– Schauplatz, Atmosphäre und zeitlicher Rahmen.
Zentrale Themen und Motive:
– Kernaussage zu menschlichen/gesellschaftlichen Problemen.
– Unterstützende Motivik.

3. Schluss
Zusammenfassung: der wichtigsten Analyseergebnisse.
Überprüfung der Deutungshypothese: Bestätigung, Präzisierung oder Differenzierung.
Einordnung und Ausblick:
– Kontext (historisch/epochal).
– Aktualität und allgemeine Bedeutung.
– Optional: Kurze, begründete persönliche Stellungnahme.`;

var structure3 = `1. Einleitung: Kontextualisierung und Hypothesenbildung
Thematische Hinführung: Allgemeiner Einstieg zum Thema oder Problem des Textes.
Basisdaten (TATT-Satz):
– Titel des Werkes
– Autor/in
– Textsorte (Kurzgeschichte, Roman, Drama, Gedicht)
– Thematik/Kernkonflikt.
Ergänzende Kontextualisierung: Erscheinungsjahr und ggf. Epochenzuordnung.
Deutungshypothese: Zentrale These in einem prägnanten Satz formulieren.
Vorgehensweise: Kurzer Ausblick auf die methodische Herangehensweise.

2. Hauptteil: Systematische Analyse und Interpretation
Inhaltliche und strukturelle Grundlagen:
– Präzise Inhaltsangabe (Präsens).
– Gliederung in Sinnabschnitte und Handlungsverlauf.
– Einordnung in das Gesamtwerk (bei Auszügen).
Aspektorientierte Textanalyse (Kernbereich):
– Figurencharakterisierung, Konstellation und Entwicklung.
– Erzähltechnik (Perspektive, Zeitgestaltung, Raumgestaltung) bzw. Dramentechnik (Aktstruktur, Regieanweisungen).
– Sprachlich-stilistische Analyse: Wortebene, Bildebene (Metaphern, Symbole), Satzebene und rhetorische Mittel.
Interpretation und Synthese:
– Deutung der Gestaltungsmittel und Wirkung.
– Zusammenhänge zwischen Form und Inhalt herausarbeiten.
– Bezug zur Deutungshypothese herstellen.
Kontextualisierung: Epochentypik, gesellschaftlicher Bezug oder biografischer Kontext.

3. Schluss: Abrundung und Reflexion
Zusammenfassung: der Hauptergebnisse.
Überprüfung der These: Bestätigung, Präzisierung oder Modifikation.
Gesamtdeutung: Abschließende Bewertung der Textaussage.
Erweiterte Reflexion (optional): Aktualität/Relevanz, Einordnung ins Gesamtwerk oder begründete persönliche Stellungnahme.

4. Methodische Anforderungen`;

var structure4 = `1. Einleitung
Textvorstellung: Autor, Titel, Textsorte (Kommentar, Essay, Rede, Zeitungsartikel), Erscheinungsjahr/Quelle.
Thema: Zentrale These des Autors präzise benennen.
Überleitung: Hinführung zur Fragestellung und Ankündigung der Analyse sowie der kritischen Erörterung.

2. Analyse des Textes (Sachliche Untersuchung)
Inhaltsangabe: Wichtigste Aussagen, Gedankengang und Argumentationslinie in eigenen Worten zusammenfassen.
Argumentationsstruktur: Hauptthese, Teilargumente, Belege/Beispiele und Schlussfolgerung identifizieren.
Sprachliche Mittel: Rhetorische Mittel (z. B. Metaphern, Ironie, Appelle) oder Statistiken finden und deren Wirkung sowie Funktion erklären.

3. Erörterung/Stellungnahme
Vorgehensweise: Eigene Meinung sachlich und begründet darlegen (dialektisch mit Pro/Contra oder linear Punkt für Punkt).
Argumentationsschema: Behauptung, Erklärung, Beispiel (aus Alltag, Schule, Gesellschaft etc.) und Bezug zur These.
Qualität: Logische Ordnung, Fachbegriffe korrekt nutzen und pro Absatz ein Argument ausarbeiten.

4. Schluss`;

var structure5 = `1. Einleitung (ca. 5–8 Sätze)
Textgrundinformationen: Autor:in, Titel, Erscheinungsjahr, literarische Epoche und Textsorte (z.B. lyrisches Gedicht, Sonett).
Thematische Hinführung: Knappe Einordnung des Hauptthemas sowie Nennung zentraler Motive oder Konflikte.
Deutungshypothese: Kernaussage in einem Satz formulieren (verdeutlicht/thematisiert/kritisiert), die als roter Faden dient.
Vorgehensweise: Kurze Ankündigung des analytischen Vorgehens.

2. Hauptteil (Aspektorientierte Analyse)
A. Inhalt und Sprechsituation:
– Paraphrase des Inhalts.
– Analyse des lyrischen Ichs (Verfassung) und der Sprechsituation (Selbstgespräch, Reflexion).
– Adressat:in und Gedankenführung (Entwicklungslinien, Wendepunkte/Volta).
B. Formale Analyse:
– Äußere Form (Strophen-/Versanzahl) und Reimschema (z.B. Kreuzreim, Paarreim).
– Metrum (Jambus, Trochäus etc.), Kadenzen (männlich/weiblich) und Enjambements.
– Besondere Gliederung (Parallelismus, Steigerung).
C. Sprachlich-stilistische Analyse (2-A-W-Prinzip):
– Wortwahl: Wortfelder, Schlüsselwörter, Konnotationen.
– Bildsprache: Metaphern, Symbole, Personifikationen, rhetorische Mittel (Anaphern, Klimax).
– Klanggestaltung: Alliteration, Onomatopoesie, Euphonie/Kakophonie.
– Syntax: Hypotaxe vs. Parataxe, Satzlängen, Inversionen.

3. Synthese und vertiefte Interpretation
Zusammenführung: Wie stützen Form und Sprache die inhaltliche Aussage?
Interpretation: Kernbotschaft, Intention und Überprüfung der Deutungshypothese.
Kontext: Epochenmerkmale (z.B. Romantik, Expressionismus) sowie historischer oder biografischer Bezug.

4. Schluss (ca. 5–8 Sätze)`;
...
function setTopicContent(Headline, StructureCodeH, StructureText){
    document.getElementById("Aufbau").innerHTML = StructureCodeH;
    currentStructureCode = StructureText;
...
async function copyPrompt() {
    // 1. Werte aus dem DOM und Variablen holen
    const topic = currentTopic;
    
    // 2. Den String mit Template Literals sauber zusammenbauen
    const promptText = `Entnehme den Text aus dem Bild und verfasse eine: ${topic}. 
Mit dieser Struktur: ${currentStructureCode}. 
Achte sprachlich darauf, dass du nicht zu viele Sätze mit vielen Nebensätzen formulierst, damit der Fließtext natürlicher wirkt, ohne die Kriterien zu missachten. 
Die Gesamtlänge des Textes soll insgesamt 4 x DIN A4 Seiten betragen.`;
    
    try {
        // 3. Moderne Clipboard API (funktioniert auf den meisten Handys)
